...
@auth_router.post(
    "/register", response_model=user_schemas.UserEncrypted, status_code=201
)
def auth_register(
    userSchema: user_schemas.UserEncrypted, db: Session = Depends(get_db)
):
...
@auth_router.get("/token/", response_model=user_schemas.User)
async def read_users_me(
    db: Session = Depends(get_db),
    token_data: user_schemas.User = Depends(get_token_data),
):
    current_user = get_user(db, email=token_data.email)
    return current_user
